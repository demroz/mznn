...
#ifndef MEMPOOL_HPP__
#define MEMPOOL_HPP__
...
class MemPool {
 ...
 public:
  ...
  T *emplace_back_multi(size_t n) {
    LOG_DEBUG("calling empalce back muilti at runtime");
    if (std::distance(_nextFreeSpace, _currentBlock->end()) < n) {
      _nextblock();
    }
    auto ret = _nextFreeSpace;
  ...
  Iterator end() {
    LOG_DEBUG("iterator end");
    return Iterator(_data.begin(), _data.end(), _currentBlock, _nextFreeSpace,
                    _nextFreeSpace);
  }
  ...
  void print_debug_info() {
    ...
    for (auto it = begin(); it != end(); ++it) {
      ...
      LOG_DEBUG("item at index: {} address: {:#x} value: {} db: {} de: {}", i,
