...
namespace aad
{
    ...
    class PoolIterator
    {
        ...
        public:
            ...
            /* getters */
            blockIterator getCurrentBlock()
            {
                return _currentBlock;
            }
            blockIterator getFirstBlock()
            {
                return _firstBlock;
            }
            blockIterator getLastBlock()
            {
                return _lastBlock;
            }
            itemIterator getCurrentItem()
            {
                return _currentItem;
            }
            itemIterator getLastItemInPool()
            {
                return _lastItemInPool;
            }
        ...
        public:
            ...
            void print_debug_info()
            {
                ...
                for(auto it = begin(); it != end(); ++it)
                {
                    ...
                    LOG_DEBUG("item at index: {} address: {:#x} value: {} db: {} de: {}",
                              i, reinterpret_cast<std::uintptr_t>(&*it), *it, dist_begin, dist_end);
                    ++i;
                    if (dist_begin < 0)
                    {
